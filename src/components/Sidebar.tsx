...
// Design haut de gamme avec flous glassmorphes, capsules actives lumineuses et contrastes garantis via classes CSS dédiées
...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <div className="flex md:hidden fixed top-0 left-0 right-0 h-[60px] backdrop-blur-lg z-50 justify-between items-center px-4 select-none mobile-header-premium">
        {/* Logo Aura */}
        ...
        <div className="flex items-center space-x-1">
          ...
          <button
            ...
            title={t("sidebar.lang_switch")}
            className="w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-300 cursor-pointer relative mobile-top-btn"
          >
            <Globe className="w-4.5 h-4.5" />
          ...
          <button
            ...
            title={theme === "dark" ? t("sidebar.theme_light") : t("sidebar.theme_dark")}
            className="w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-300 cursor-pointer mobile-top-btn"
          >
            <Palette className="w-4.5 h-4.5" />
          </button>
          ...
          <button
            onClick={logout}
            title={t("sidebar.logout")}
            className="w-9 h-9 rounded-xl flex items-center justify-center hover:text-red-500 hover:bg-red-500/10 transition-all duration-300 cursor-pointer mobile-top-btn"
          >
            <LogOut className="w-4.5 h-4.5" />
          </button>
        </div>
      ...
      <nav className="flex md:hidden fixed bottom-0 left-0 right-0 h-[68px] backdrop-blur-lg z-50 justify-around items-center px-4 select-none mobile-nav-premium">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = pathname === item.href || (item.href !== "/" && pathname?.startsWith(item.href));
          const isMessages = item.href === "/messages";
          ...
          return (
            <Link
              key={item.name}
              href={item.href}
              className={`flex flex-col items-center justify-center flex-1 py-1 relative cursor-pointer mobile-nav-item ${
                isActive ? "active" : ""
              }`}
            >
              <div className="p-2.5 rounded-xl transition-all duration-300 flex items-center justify-center relative mobile-nav-item-icon-container">
